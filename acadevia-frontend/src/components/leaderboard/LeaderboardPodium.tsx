...
const LeaderboardPodium: React.FC<LeaderboardPodiumProps> = ({ top3, className, onSelectStudent }) => {
      ...
      {podiumOrder.map((idx, i) => {
        ...
        return (
          <motion.div
            ...
            onClick={() => entry.userId && onSelectStudent?.(entry.userId)}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => {
              if ((e.key === 'Enter' || e.key === ' ') && entry.userId) {
                onSelectStudent?.(entry.userId);
              }
            }}
            className="flex flex-col items-center w-28 cursor-pointer group select-none"
          >
            ...
            <p className="text-xs font-semibold text-center truncate w-full group-hover:text-primary transition-colors">{entry.name}</p>
            <p className="text-[10px] text-gray-500 mb-1">{entry.xp.toLocaleString()} XP</p>
            <div className={cn('w-full rounded-t-xl flex items-start justify-center pt-2 group-hover:brightness-105 transition-all', podiumHeights[i], podiumColors[idx])}>
