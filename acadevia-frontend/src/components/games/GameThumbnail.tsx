...
export const GameThumbnail: React.FC<{ game: GameDefinition; className?: string }> = ({ game, className = '' }) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-br ${game.colors} ${className}`} aria-label={`${game.title} artwork`}>
      ...
      {game.id === 'history-quest' ? (
        <div className="absolute right-4 bottom-2 text-4xl opacity-80 filter drop-shadow">
          🏺📜
        </div>
      ) : game.id === 'science-lab' ? (
        <div className="absolute right-4 bottom-2 text-4xl opacity-80 filter drop-shadow">
      ...
      {(game.id === 'history-quest' || game.id === 'science-lab') && (
        <Icon className="absolute bottom-4 left-5 h-14 w-14 text-white drop-shadow-lg" strokeWidth={1.4} />
      )}
